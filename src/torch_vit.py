# modified from pytorch vision to enable forward hook to retrieve attention weights
import math
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = self.ln_1(input)
        # added by me addention weights, we want to return the attention weights for each head
        x, attention_weights = self.self_attention(x, x, x, need_weights=True, average_attn_weights=False)
